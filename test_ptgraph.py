...
class TestSendRecvBuiltin:
    ...
    def test_sum_reduce(self):
        ...
        g.send_and_recv(torch.arange(3), self._msg_copy_src, "sum")
    ...
    def test_mean_reduce(self):
        ...
        g.send_and_recv(torch.arange(3), self._msg_copy_src, "mean")
    ...
    def test_non_receiving_nodes_preserved(self):
        ...
        g.send_and_recv(torch.arange(1), self._msg_copy_src, "sum")
...
class TestBalaGoyalSmoke:
    ...
    def test_multiple_steps(self):
        ...
        for step in range(5):
            ...
            edges = g.filter_edges(lambda e: e.src["payoffs"][:, 1] > 0.3)
            if edges.shape[0] > 0:
                g.send_and_recv(
                    edges,
        ...
        for step in range(3):
            ...
            def applyfn(nodes):
                ...
                posterior = (
                    prior
                    * likelihood
                    / (prior * likelihood + (1 - prior) * (1 - likelihood)).clamp(
                        min=1e-8
                    )
                )
...
class TestUnreliableOpsPatterns:
    ...
    def test_unreliable_filter_then_send_recv(self):
        ...
        def filterfn(edges):
            return (
                torch.gt(edges.src["payoffs"][:, 1], 0.0) * edges.src["reliability"]
            ).bool()
